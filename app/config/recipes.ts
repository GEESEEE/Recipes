...
const recipeFieldSorter = (fields: string[]) => (r1: Recipe, r2: Recipe) => fields.map(field => {
    let dir = 1;
    if (field[0] === '-') { dir = -1; field=field.substring(1) }
    ...
    if (r1Value instanceof Array && r2Value instanceof Array) {
        return r1Value.length > r2Value.length ? dir : r1Value.length < r2Value.length ? -(dir) : 0
    }
...
export function applySort(recipes: Recipe[], sort: string[]): Recipe[] {
    if (sort.length > 0) {
        const fields = sort.map(s => {
            ...
            const field = sortFieldMap[(reverse ? s.substring(1) : s) as RecipeSortType]
